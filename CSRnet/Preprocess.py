...
def gaussian_filter_density(gt):
    ...
    for i, pt in enumerate(pts):#枚举，返回序列（索引，数据）
        ...
        if sigma in [float("-inf"),float("inf")]:
            sigma =  float("nan")
...
test_data = os.path.join(root,'test/test_images')
...
for img_path in glob.glob(os.path.join(test_data, '*.jpg')):
    img_paths.append(img_path)
print(len(img_paths))

mat = io.loadmat('/home/bedant/CSRnet/ann.mat')
...
for img_path in tqdm(img_paths):#tqdm显示进度条
    ...
    gt = mat[((img_path.split(test_data)[1]).split('.')[0]).replace('/','')]
    ...
    file_path = img_path.replace('.jpg','.h5').replace('test_images', 'den')
